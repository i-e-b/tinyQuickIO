namespace IntegrationTests
{
    ...
    public class working_with_long_paths 
    {
        ...
        public static void directory_paths_with_more_than_255_characters_are_supported() {
            ...
            File.CreateDirectory(deepPath, recursive: true);

            Assert.IsTrue(File.Exists(deepPath), "New directory was not created or could not be found");
            File.DeleteDirectory(new DirectoryDetail(TempRoot + "\\QIO"), recursive: true);
        }
        ...
        public static void files_can_be_created_and_written_and_read_and_copied_in_very_long_paths()
        {
            ...
            File.CreateDirectory(new PathInfo(path), recursive: true);
            ...
            // write a file
            using (var fs = File.OpenFileStream(srcFile, FileAccess.Write, FileMode.Create, FileShare.None)) {
            ...
            Assert.True(File.Exists(srcFile), "Source file can't be found (didn't write correctly?)");
            Assert.True(File.CopyFile(srcFile, dstFile), "Failed to copy file");
            Assert.True(File.Exists(dstFile), "Target file can't be found");

            // Check the contents
            using (var fs = File.OpenFileStream(srcFile, FileAccess.Read))
            {
                var buf = new byte[8];
